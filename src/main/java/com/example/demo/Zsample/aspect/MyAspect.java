package com.example.demo.Zsample.aspect;
...
public class MyAspect {
    ...
    @Before("execution (* com.example.demo.Zsample.jdbc.SonyPrinter.print(..))")
    public void before() {
        System.out.println("I'm before.");
    }

    //在切入點SonyPrinter.print()執行之後執行
    @After("execution (* com.example.demo.Zsample.jdbc.SonyPrinter.print(..))")
    public void after() {
        System.out.println("I'm after.");
    }

    //在切入點SonyPrinter.myFunction()執行的前後都執行, 但因myFunction非interface裡面的function不確定是否因為這樣所以無法透過左方的method連結到
    @Around("execution (* com.example.demo.Zsample.jdbc.SonyPrinter.myFunction(..))")
